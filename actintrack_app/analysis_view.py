    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self.tbl_breed_summary = self._make_table(
            [
                ...
                "Avg General Movement (µm/s)",
                "Avg Toward Nucleus (µm/s)",
                "Avg F-actin Orientation 0–90°",
                "Samples with Orientation",
                "Avg OF General Movement (µm/s)",
                "Samples with OF",
                "Legacy Avg Downward (µm/s)",
                "Legacy Avg Motion Index",
                "Std Dev General Movement",
                "Legacy Std Dev Downward",
        ...
        self.tbl_sample_details = self._make_table(
            [
                ...
                "Toward Nucleus (µm/s)",
                "F-actin Orientation Median 0–90°",
                "Angle Measurements",
    ...
    def _fill_breed_summary(self, rows: list[BreedSummaryRow]) -> None:
        ...
        for r, row in enumerate(rows):
            values: list[tuple[str, Any, bool]] = [
                ...
                (
                    _fmt_float(row.avg_orientation_median_deg),
                    row.avg_orientation_median_deg,
                    True,
                ),
                (
                    str(row.samples_with_orientation_results),
                    row.samples_with_orientation_results,
                    True,
                ),
                (_fmt_float(row.avg_of_general_movement), row.avg_of_general_movement, True),
                (str(row.samples_with_of_results), row.samples_with_of_results, True),
    ...
    def _fill_sample_details(self, rows: list[SampleAnalysisRow]) -> None:
        ...
        for r, row in enumerate(rows):
            ...
            values: list[tuple[str, Any, bool]] = [
                ...
                (
                    _fmt_int(m.orientation_measurement_count),
                    m.orientation_measurement_count,
                    True,
                ),
